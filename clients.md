...
**Solution:** Built a fully custom Excel/VBA system integrated with WinWedge to control ticketing workflows end-to-end (weighing, testing, ticket creation, reporting). Included Outlook-based automated PDF email delivery, dynamic customer lookup, load splitting, and a flexible reporting engine with filter-driven outputs.
...
**Solution:** Developed VBA macros to enable bi-directional synchronization between a master sheet and multiple category tabs, including controlled record creation and formatting preservation.
...
**Solution:** Built a dashboard-driven Excel tool with button-triggered macros to log daily inputs and visualize KPIs (revenue pacing, required daily targets, profit per job/truck) using charts and summary metrics.
...
**Solution:** Built a custom Airtable system integrated with Shopify (via SyncBase) to track orders through multi-branch workflows, automate status changes, and manage customer communication tied to each stage of the process.
...
**Solution:** Developed a CRM with email forwarding to Airtable (custom parsing via JavaScript), automatic contact matching/creation, and touchpoint logging. Integrated Google Calendar to generate touchpoints from meetings and built automated daily task summary emails.
...
**Solution:** Built a workflow-driven Airtable system with role-specific interfaces, form-based inputs, and automated email notifications to move projects through each stage while preventing user error.
...
**Solution:** Migrated system to Airtable with MiniExtensions to enable “forms-within-forms,” allowing users to create related records (e.g., clients + jobs) in a single flow without understanding database structure.
...
**Solution:** Built an automation pipeline using Make.com to process Clockify timesheet data through Airtable and generate invoices in Bexio automatically.
...
**Solution:** Built a multi-sheet ecosystem with interconnected dashboards, inventory tracking, and a formula-driven task prioritization engine (based on timing, seasonality, and weather conditions). Integrated OpenWeatherMap API and Minnesota DOT alerts, and added Apps Script automations for syncing and notifications.
...
**Solution:** Developed a scheduling platform using Apps Script to expand recurring services into calendar-based bookings, with a real-time, color-coded schedule grid that combines availability and assignments and flags conflicts.
...
**Solution:** Built a menu-driven system where users select items and generate a fully formatted prep list via a custom Apps Script menu, reducing planning time from hours to minutes.
...
## Data Visualization & Chart Recreation
...
**Solution:** Reconstructed underlying data using manual estimation, AI tools, and visual tracing, then built polished charts in Google Sheets/Excel and enhanced visuals with Canva overlays for features not natively supported.
...
**Solution:** Delivered clean, presentation-ready charts in Excel and Google Sheets under tight deadlines.
